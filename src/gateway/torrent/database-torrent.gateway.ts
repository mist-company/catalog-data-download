...
import type { LoggerHelper } from "../../helper/logger/logger.helper";
import { BaseLoggerHelper } from "../../helper/logger/base-logger.helper";
import type { BaseTorrentGatewayListInput } from "./base-torrent.gateway";
...
export class DatabaseTorrentGateway {
	...
	async get(input: { infoHash: string }): Promise<Torrent> {
		...
		const torrent = await collection.findOne<Torrent>({
