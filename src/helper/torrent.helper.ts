...
export class TorrentHelper {
  static buildQueriesFromTitle(title: Title, episode?: Title): string[] {
    ...
    if (episode) {
      ...
      queries.push(`${title.primaryTitle} S${seasonNumber}E${episodeNumber} ${episode.originalTitle}`);
      queries.push(`${title.originalTitle} S${seasonNumber}E${episodeNumber} ${episode.originalTitle}`);
    } else {
