...
export function buildQueriesFromTitle(title: Title, episode?: Title): string[] {
	if (
		["tvSeries", "tvMiniSeries", "tvEpisode"].includes(title.titleType) &&
		!episode
	) {
	...
	if (episode) {
		const seasonNumber = episode.imdbId.season;
		const episodeNumber = episode.imdbId.episode;
...
export function extractCleanTitle(torrentName: string): string {
	...
	const movieMatch = balanced.match(
		/^(.*?)\s*(?:\(|\[)?(19\d{2}|20\d{2})(?:\)|\])?/,
	);
	if (movieMatch) {
