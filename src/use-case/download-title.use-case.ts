...
export class DownloadTitleUseCase {
  ...
  constructor(
    ...
    @inject(BaseTorrentClientGateway) torrentClientGateway: BaseTorrentClientGateway,
  ...
  async execute(input: DownloadTitleUseCaseInput): Promise<void> {
    const download = await this.#downloadGateway.get({ infoHash: input.infoHash });
    if (download) {
      this.#logger.info({ input }, 'download already exists');
      const { path, status } = await this.#torrentClientGateway.get(input.infoHash);
      await this.#downloadGateway.update({ ...download, path, status });
      this.#logger.info({ input, path, status }, 'download already exists, updated');
      return;
    }
