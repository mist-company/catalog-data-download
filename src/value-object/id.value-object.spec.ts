...
describe("IdValueObject", () => {
	...
	it("returns the value property", () => {
		const titleId = new IdValueObject("tt000000:1:2");
		expect(titleId.value).toBe("tt000000:1:2");
	});

	it("parses the value to lowercase", () => {
		const titleId = new IdValueObject("TT000000:1:1");
		expect(titleId.value).toBe("tt000000:1:1");
		expect(titleId.id).toBe("tt000000");
	});
});
